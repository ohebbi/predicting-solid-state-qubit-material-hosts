...
def runFeaturizeMP():
    ...
    MP_entries = pd.read_csv("data/databases/initialDataMP/MP/MP_FLAGBIGFILE.csv", sep=",")
    print(MP_entries.shape)
    fileName = "data/databases/initialDataMP/MP/MP_electronicFeaturized.csv"
    featurize_by_material_id(MP_entries["material_id"].values, api_key, fileName)

def concatenateReQueueFeaturizers(directory):
    if os.path.exists(directory+"/MP_electronicFeaturized.csv"):
    ...
    MP_featurized_portion = pd.read_csv(directory+"/MP_electronicFeaturized.csv", sep=",")
    MP_featurized = pd.concat([MP_featurized,MP_featurized_portion]).reset_index(drop=True)
    MP_featurized.to_csv(directory+"/MP_electronicFeaturized.csv",sep=",")
...
def reQueueMPFeaturizer():
    ...
    MP_featurized = pd.read_csv(directory+"/MP_electronicFeaturized.csv", sep=",")
    ...
    api_key = "b7RtVfJTsUg6TK8E"
    fileName = directory+"/MP_electronicFeaturized.csv"
